...
osThreadId CoaP_client_tasHandle;
...
int main(void)
{
  ...
  sensorData=xQueueCreate(200,sizeof(char *));

  /* USER CODE END RTOS_QUEUES */

  /* Create the thread(s) */
  /* definition and creation of CoaP_client_tas */
  osThreadDef(CoaP_client_tas, StartDefaultTask, osPriorityNormal, 0, 3*1024);
  CoaP_client_tasHandle = osThreadCreate(osThread(CoaP_client_tas), NULL);

  /* definition and creation of sensor_task */
  osThreadDef(sensor_task, StartTask02, osPriorityIdle, 0, 1024);
...
/**
  * @brief  Function implementing the CoaP_client_tas thread.
...
void StartTask02(void const * argument)
{
  ...
  for(;;)
  {
	osDelay(500);
		...
		osMutexWait(printf_MutexHandle, portMAX_DELAY);

		printf("\n\r %s\n\r",sbuffer);
		osMutexRelease(printf_MutexHandle);
		xQueueSend(sensorData,&sbuffer,portMAX_DELAY);
  }
